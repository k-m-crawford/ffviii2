...
func _unhandled_input(event):
	...
	if event.is_action_pressed("ui_accept") and is_on_floor():
		if alert_bubble_instance:
			discoverable_raycast.get_collider().on_trigger()
...
func check_for_discoverables():
	var hit = discoverable_raycast.get_collider()
	
	if hit and not alert_bubble_instance:
		if hit.discoverable:
			alert_bubble_instance = alert_bubble.instantiate()
			add_child(alert_bubble_instance)
	elif not discoverable_raycast.get_collider() and alert_bubble_instance:
		alert_bubble_instance.get_node("AnimationPlayer").play("PopClosed") # unloads self
		alert_bubble_instance = null
