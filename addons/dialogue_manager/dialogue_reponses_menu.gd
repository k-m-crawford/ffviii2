...
func set_responses(next_responses: Array, best_size: Vector2) -> float:
	...
	if _responses.size() > 0:
		var longest_response_width: float = best_size.x
		for response in _responses:
			...
			var width: float = item.get_theme_font("normal_font").get_string_size(item.text, HORIZONTAL_ALIGNMENT_LEFT, -1, 30).x + 10
			if width > longest_response_width:
				longest_response_width = width
			add_child(item)

		_configure_focus()
		
		return longest_response_width
	return 0
...
func _on_response_gui_input(event: InputEvent, item: Control, item_index: int) -> void:
	...
	if event.is_action_pressed("ui_accept") and item in get_menu_items():
